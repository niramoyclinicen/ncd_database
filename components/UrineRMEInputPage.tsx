...
const UrineRMEInputPage: React.FC<any> = ({ results: initialResults, onSaveOverride, disabled, patient, invoice, doctors, employees, technologistId, consultantId, isEmbedded }) => {
  ...
  const updateField = (f: keyof UrineRMEResults, v: string) => {
  ...
  const InputRow = ({ label, field, options, isLast }: any) => {
    return (
      ...
      <div className={`flex-1 ${isEmbedded ? '' : 'overflow-y-auto p-4 flex justify-center custom-scrollbar bg-slate-200/50'}`}>
        <div className={`${isEmbedded ? 'w-full' : 'bg-white w-full max-w-[820px] shadow-2xl p-12'} flex flex-col font-serif relative`}>
            ...
            <div className="flex-1">
              <div className="flex flex-col border-2 border-black rounded-xl overflow-hidden bg-white">
                
                {/* 1. PHYSICAL SECTION (TOP) */}
                ...
                {/* 3. MICROSCOPIC SECTION (BOTTOM) */}
                <div>
                  <SectionHeader title="Microscopic Examination" />
                  <div className="grid grid-cols-2 divide-x border-black">
                    <div className="flex flex-col">
                      <InputRow label="Epithelial Cells" field="epithelialCells" />
                      <InputRow label="Pus Cells" field="pusCells" />
                      <InputRow label="R.B.C" field="rbc" />
                    ...
                    <div className="flex flex-col">
                      ...
                      <InputRow label="Crystals" field="crystals" />
              ...
              <div className="mt-10 pt-6 border-t-2 border-black">
                  ...
                  />
                  <p className="hidden print:block text-[18px] font-black italic whitespace-pre-wrap text-black">{localResults.others || 'Urine routine examination reveals normal findings. No evidence of urinary tract infection or metabolic disorder detected in this sample.'}</p>
              </div>
            </div>
        </div>
      </div>
